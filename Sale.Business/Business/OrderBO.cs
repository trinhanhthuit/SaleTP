...
{
    public class OrderBO : Repository<Order>, IOrder
    {
        ...
        //TODO
        public DataBackEndRespone GetOrder(string searchString, int isActive, string langId, int pageIndex, int pageSize)
        {
            try
            {
        ...
        public bool SaveOrder(Order order, List<OrderDetail> orderDetails, Customer cus)
        {
            try
            {
                using (var db = new DBEntities())
                {
                    ...
                    else
                    {
                        ...
                        var details= db.OrderDetails.Where(m => m.OrderID == order.OrderID ).ToList();
